 ...
 * @LastEditTime: 2023-02-14 16:26:27
    ...
    <var-popup :show="show" :default-style="false" @close="close" @click-overlay="close" position="bottom">
      <div class="popup text-center bg-white">
        ...
        <div class="px-20px bg-whi" @click.capture="changeHandle">
          <div class="cat_item" :class="isActive('全部') ? activeClass : commonClass"  data-index="-1">全部</div>
          <div class="flex justify-around flex-wrap">
            <div class="cat_item" :class="isActive(item) ? activeClass : commonClass"  :data-index="index" v-for="(item, index) in list" :key="index">{{ item }}</div>
          </div>
        </div>
      </div>
    </var-popup>
...
const activeClass = computed(()=>{
  return [`text-[${props.activeColor}]`,`bg-[${props.activeBgColor}]`]
})
