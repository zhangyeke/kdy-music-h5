 ...
 * @LastEditTime: 2022-05-25 17:42:48
...
<template>
  <div class="single bg-white px-10px">
    ...
    <div class="single_list font-500" v-if="searchStore.song_list.length">
      <div class="single_item border_b_solid_1 py-10px flex items-center" v-for="(item, index) in searchStore.song_list" :key="item.id">
        <div class="w-100/80" v-ripple>
          ...
          <div class="text-10px text-[#666] mt-5px">
            ...
            <div class=" mt-5px truncate">
              <div class="" v-if="item[aliasKey]?.length"><span v-for="(e, i) in item[aliasKey]" :key="i">{{ e }}</span></div>
        ...
        <div class="w-20/100 flex items-center justify-end ">
          <div @click.stop="" v-if="item.mv" v-ripple >
            <var-icon name="bofang1" namespace="kdy-icon" color="#333" :size="tool.px2vw(20)" />
          </div>
          <div @click.stop="" class="px-10px" v-ripple>
...
let l = computed(()=>{
  if(searchStore.type == 1018){
    return searchStore.song_list
  }else{
